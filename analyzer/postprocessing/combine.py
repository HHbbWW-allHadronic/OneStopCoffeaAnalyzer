...
if command -v combine &> /dev/null; then
    ...
    function run_combine() {
       ...
       apptainer exec -B /cvmfs -B $(pwd -P) -B /cvmfs --pwd $(pwd -P)  $IMAGE /bin/bash -c 'source /cvmfs/cms.cern.ch/cmsset_default.sh && pushd $PWD && cd /home/cmsusr/CMSSW_14_1_0_pre4/ && eval $(scramv1 runtime -sh) && popd && exec \"$@\"' -- $@
    }
    RUN_CMD="run_combine"
